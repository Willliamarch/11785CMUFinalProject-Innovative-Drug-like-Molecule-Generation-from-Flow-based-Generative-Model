...
class PDB_file():
    ...
    def parseFile(self):
        PDBInfo = []
        with open('./pdb_loader/' + self.PDB, 'r') as rf:
    ...
    def calMissSeq(self):
        ...
        totalMissing = totalMissing / (self.totalLength + 1e-7)
...
def writeBindingSites(allBinding, fileName='./Final_Binding45angstroms/cleanedAllBindingSites.txt'):
    with open(fileName, 'w') as wf:
...
def writeBindingStrength(allBinding, fileName = './Final_Binding45angstroms/cleanBindingStrength.txt'):
    with open(fileName, 'w') as wf:
        for pdb in allBinding.keys():
            for smal in allBinding[pdb].keys():
                for chain in allBinding[pdb][smal].keys():
...
def writeMissingResidue(allMissingProportion, filename='MissingSeqProportion.txt'):
    with open('./Final_Binding45angstroms/' + filename, 'w') as wf:
...
def writeResidue_GNM(allResidue_GNM, filename='./Final_Binding45angstroms/Residue45A_CA.txt'):
    with open(filename, 'w') as wf:
...
def calcUnitLength(currLength):
    totalLength = 0
    for item in currLength:
    	totalLength += int(item[1])
    return totalLength

def main():
    # currPDB = PDB_file('1dos.pdb1')

    # finalList = readList('finalList_withoutAA.txt')
    # finalList = readList('RestPDBList.txt')
    pdb_names = os.listdir('./pdb_loader/')
    # print (len(pdb_names))
    ...
    for i in range (len(drug_PDB_file)):
        print (str(i+1) + ' ' + drug_PDB_file[i] + ' Start!')
        curr = drug_PDB_file[i]
        # curr = '1m0w'
        ...
        currLen = calcUnitLength(currPDB.seqlength)
        if 'irvv' not in drug_PDB_file[i] and currLen <= 3000: # max(BindingSubUnitLength(currPDB.calBinding(), currPDB.seqlength).values()) <= 1000:
            ...
            try:
                ...
                for key in currPDB.ligand.keys():
                    ...
                    print ('skip curr sturtcure')
             
        else:
            print ('skip curr structure!')
    writediction_twoDArray(allProteinLength, 'proteinLength45A_sup.txt')
    writediction_twoDArray(allProteinNames, 'proteinNames45A_sup.txt')
    writediction_twoDArray(allHETNAM, 'ligandNames45A_sup.txt')
    writeBindingSites(allBinding, fileName='./Final_Binding45angstroms/BindingSites45A_ID_basedLigand2.txt')
    writeMissingResidue(allMissingProportion)
    writeResidue_GNM(allResidue_GNM, './Final_Binding45angstroms/Residue45A_CA.txt')
    writeBindingStrength(allBinding, fileName='./Final_Binding45angstroms/BindingStrength45A_ID_basedLigand2.txt')
